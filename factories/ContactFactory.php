...
namespace yiioverflow\admincontacts\factories;

use yiioverflow\admincontacts\helpers\GoogleHelper;
use yiioverflow\admincontacts\objects\Contact;
...
abstract class ContactFactory
{
    ...
    public static function create($name, $phoneNumber, $emailAddress, $note, $customConfig = NULL, $initial=NULL,$whatsappNumber=NULL)
    {
        $doc = new \DOMDocument();
        ...
        $title = $doc->createElement('gd:name', $name);
        $title->setAttribute('givenName', $name);
        $title->setAttribute('familyName', $initial);
       // $title->setAttribute('fullName', $name);
        $entry->appendChild($title);

        $email = $doc->createElement('gd:email', $emailAddress);
        ...
        {
            $contact = $doc->createElement('gd:phoneNumber', $whatsappNumber);
            $contact->setAttribute('rel', 'http://schemas.google.com/g/2005#work');
            $entry->appendChild($contact);
        }
